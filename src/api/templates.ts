...
export async function fetchOutboundSamplePdfBlob(
  docCode: string,
  options?: {
    scenario?: string;
    statusCode?: string;
  ...
  if (!res.ok) {
    let errorMsg = 'Error al renderizar la muestra de salida/despacho en PDF.';
    try {
      const errorData = await res.json();
      errorMsg = errorData.message || errorMsg;
    } catch {
...
export async function downloadOutboundSamplePdf(
  docCode: string,
  options?: {
    scenario?: string;
    statusCode?: string;
  ...
  if (!res.ok) {
    let errorMsg = 'Error al descargar la muestra de salida/despacho.';
    try {
      const errorData = await res.json();
      errorMsg = errorData.message || errorMsg;
    } catch {
